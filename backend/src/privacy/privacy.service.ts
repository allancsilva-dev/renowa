...
const ANONYMIZED_FIELDS = ['razao_social','cnpj','email','tel','endereco','bairro','cidade','uf','cep','contato','inscricao_estadual','suframa','pgt_padrao','prazo','local_entrega','observacao'];

@Injectable()
export class PrivacyService {
  constructor(@InjectRepository(LgpdRequest) private readonly repo: Repository<LgpdRequest>,
    private readonly dataSource: DataSource, private readonly audit: AuditService) {}
  ...
  async create(dto: CreateLgpdRequestDto, user: RequestUser) {
    const subject = await this.dataSource.query('SELECT uuid FROM clientes WHERE tenant_id = $1 AND uuid = $2', [user.tenantId, dto.subjectUuid]);
    if (!subject[0]) throw new NotFoundException('Titular não encontrado neste tenant.');
  ...
  async execute(requestUuid: string, user: RequestUser) {
    try {
      ...
      if (!request) throw new NotFoundException('Solicitação LGPD não encontrada.');
      if (request.status === 'COMPLETED') return request;
      if (request.status !== 'APPROVED') throw new ConflictException('Solicitação precisa estar aprovada.');
      request.status = 'IN_PROGRESS'; await repo.save(request);
      ...
      if (request.request_type === 'ERASURE') {
        ...
        await this.audit.record({ tenantId: user.tenantId, actor: user, action: 'DELETE', resourceType: 'cliente',
          resourceUuid: request.subject_uuid, fields: ANONYMIZED_FIELDS, purpose: 'Execução de direito de apagamento aprovado' }, manager);
      } else {
        const rows = await manager.query(`SELECT uuid, razao_social, cnpj, email, tel, endereco, bairro, cidade, uf,
          cep, contato, inscricao_estadual, suframa, created_at, updated_at FROM clientes WHERE tenant_id = $1 AND uuid = $2`,
          [user.tenantId, request.subject_uuid]);
        if (!rows[0]) throw new NotFoundException('Titular não encontrado.');
        ...
        await this.audit.record({ tenantId: user.tenantId, actor: user, action: 'EXPORT', resourceType: 'cliente',
          resourceUuid: request.subject_uuid, fields: Object.keys(rows[0]), purpose: 'Portabilidade aprovada pelo titular' }, manager);
      }
      request.status = 'COMPLETED'; request.completed_at = new Date(); request.failure_reason = null;
  ...
  private async transition(requestUuid: string, tenantId: string, allowed: string[], status: LgpdRequest['status'], patch: Partial<LgpdRequest>) {
    ...
    if (!allowed.includes(request.status)) throw new ConflictException(`Transição inválida: ${request.status} para ${status}.`);
    Object.assign(request, patch, { status }); return this.repo.save(request);
  }
}
