...
_Última atualização: 2026-07-22 (parte 2) — seções "Módulos principais", "Fluxo principal do produto" e "Limitações conhecidas" atualizadas após a implementação "Fluxo Comercial Completo" (pedidos → faturamento → comissão → caixa), registrada em [PROBLEM_LEDGER.md](PROBLEM_LEDGER.md) e [REVIEW_REPORTS/2026-07-22_fullstack_implementation_fluxo-comercial-completo.md](REVIEW_REPORTS/2026-07-22_fullstack_implementation_fluxo-comercial-completo.md). **Nota importante:** essa implementação está inteira no working tree, sem nenhum commit, no momento desta atualização — confirmado por `git status`._
...
- **Importação de produtos:** `POST /produtos/importacao` (multipart, lib `xlsx`) para cadastro em lote.
...
- Schema de produção vem só das migrations, mas dev usa `synchronize` → **divergência dev↔prod** ([PROB-0004](PROBLEM_LEDGER.md)). **Risco confirmado na prática em 2026-07-22:** `synchronize:true` em dev (`backend/src/app.module.ts:44-46`, ativo sempre que `NODE_ENV !== 'production'`) já dropou silenciosamente 4 invariantes recém-criadas por migration SQL (2 `CHECK` constraints, 2 índices únicos parciais) assim que as entities TypeORM correspondentes foram editadas com um processo `watch` ativo — CHECK constraints e índices parciais `WHERE ...` não têm decorator equivalente em TypeORM, então `synchronize` os trata como "extra" e remove. Ver [PROB-0059](PROBLEM_LEDGER.md), ABERTO, decisão de arquitetura pendente com `software-architect`.
- **Drift de triggers `updated_at` em dev (achado 2026-07-22):** a migration `0020_utc_timestamps_db_authority.sql` está registrada como aplicada em `schema_migrations`, mas a função `public.set_updated_at()` e os triggers `trg_set_updated_at` que ela deveria criar **não existem hoje no Postgres de dev** — `updated_at` de tabelas antigas (`clientes`, `fornecedores`, `pedidos`, `comissoes`, `tenant_roles`, etc.) só é setado no INSERT, nunca em UPDATE. Provável mesma causa do ponto acima (`synchronize`). Não verificado se o drift existe também em staging/produção. Ver [PROB-0060](PROBLEM_LEDGER.md), ABERTO.
