...
- **A suíte automatizada não exercita nenhuma invariante de banco.** Toda a suíte de `faturamento`/`finance`/`orders` é mock puro (`jest.fn()` sobre repositórios); nada roda contra Postgres. Os **260** testes verdes do backend (2026-07-22, era 236 no commit `d91b9b3`) não provam que locks pessimistas, FKs compostas, índices únicos parciais e CHECKs funcionem — a asserção "duas notas concorrentes no mesmo pedido serializam" é inverificável hoje. Ver [BACKLOG-0028](BACKLOG.md).
