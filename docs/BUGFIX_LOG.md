...
### FIX-0031 — RBAC fechado ponta a ponta: exclusão de perfil revoga acesso, e a tela deixou de ser vitrine
- **Data:** 2026-08-26
- **Fecha:** PROB-0084 · BACKLOG-0081 e BACKLOG-0085 (validação pela UI nos dois ramos, executada em 2026-08-26)
- **Área:** segurança / backend / frontend
- **Sintoma:** três coisas, do mesmo assunto. (1) Excluir perfil não revogava nada — quem estava vinculado continuava passando por todo `PermissionGuard`, e `GET /auth/me` seguia devolvendo as permissões fantasma. (2) A tela de Usuários só oferecia os `ROLE_TEMPLATES`: um perfil sob medida criado em Perfis **não era atribuível** a ninguém, o que tornava a tela de Perfis parcialmente decorativa. (3) O frontend gateava por permissão só onde alguém lembrou: ~19 de ~26 botões de escrita sem gate, as rotas de clientes/pedidos/produtos/fornecedores/transporte/financeiro sem `permission`, e a Sidebar filtrando 2 de 9 itens — quem não podia via tudo e descobria o 403 no submit.
- **Causa raiz:** ver PROB-0084 para o caminho crítico (soft-delete + `ON DELETE CASCADE` que nunca dispara + leitura sem filtro de role viva). Os demais achados vêm da auditoria de RBAC verificada item a item contra o código na mesma data.
- **Correção — backend:**
  - `permissions.service.ts` — `listEffectiveForRole` virou QueryBuilder com `innerJoin` em `TenantRole` exigindo `role.active = true AND role.deleted_at IS NULL`. É o ponto de estrangulamento único: atende o guard e o `/auth/me`, e cobre inclusive os perfis já excluídos **antes** da correção.
  - `permission.guard.ts` — nega também quando `localUser.active === false`, fechando a janela em que o access token de um usuário desativado continua válido até expirar.
  - `roles.service.ts#deleteRole` — reescrito em transação: recusa com **409 ConflictException** citando a contagem se houver `local_users` ativos no perfil (decisão do usuário: **fail-closed**, mover usuário é ato deliberado do admin, não efeito colateral de exclusão); sem vinculados, apaga as linhas de `tenant_role_permissions` do par `(tenantId, roleId)` **antes** do soft-delete. A auditoria ganhou `metadata.revokedPermissionCount`.
  - `roles.service.ts#updateRole` — o rename agora propaga, na mesma transação, para `usuarios.roles` (jsonb) e incrementa `access_token_version` dos vinculados, reusando o padrão de `users.service.ts`. Decisão do usuário: sincronizar agora; eliminar `usuarios.roles` fica para depois (BACKLOG-0083).
  - `create-role.dto.ts` / `update-role.dto.ts` — `@Transform` de trim + `@IsNotEmpty()` + `@MinLength(2)`. O trim tem que vir antes da validação: `@IsNotEmpty()` sozinho aceita `"   "`, que o `normalizeName` do service transformaria em nome vazio.
  - `users.service.ts` — `active: true` no `findOne` do catch de `23505` em `ensureTenantRoleWith`. Sem isso, a corrida devolvia role soft-deleted e vinculava usuário novo a perfil morto (achado novo, não estava no relatório de auditoria).
- **Correção — frontend:**
  - `lib/authorization.ts` — removido o bypass `isAdmin(roles) ||` de `hasPermission`. Ele liberava botão que a API recusa: o backend não tem bypass por nome de role, então um perfil sob medida chamado `admin`, sem permissão nenhuma, abria o produto inteiro na tela. `isAdmin`/`hasRole` seguem existindo para uso cosmético. A prop `adminOnly` de `ProtectedRoute` era código morto (nenhuma rota usava) e foi removida.
  - Novo `components/Can.tsx` — gate declarativo de ação (`permission: string | string[]`, `mode: 'all' | 'any'`, `fallback`), espelhando `@RequirePermission` do backend.
  - `App.tsx` — `permission` nas rotas de módulo (`clientes.ver`, `pedidos.ver`, `produtos.ver`, `fornecedores.ver`, `transportadoras.ver`, `financeiro.ver`) e nas rotas `novo`/`editar` de clientes, produtos e fornecedores. Dashboard e o shell de `configuracoes` seguem só autenticados — **não existe slug para eles no catálogo** (ver BACKLOG-0082).
  - `Sidebar.tsx` — `permission` nos 7 itens que faltavam, inclusive `/financeiro`.
  - ~19 botões de escrita envolvidos em `<Can>` em Dashboard, Pedidos, PedidoDetalhe, Clientes, Produtos, Fornecedores, Transporte e Financeiro.
  - `UsuariosPage.tsx` — o select de perfil passou a unir `GET /roles` (perfis materializados do tenant, inclusive os sob medida) com `ROLE_TEMPLATES`, deduplicado por nome. Falha do fetch cai nos templates com aviso `role='status'`, sem travar a criação de usuário. A montagem da lista saiu para `lib/roleOptions.ts` (`mergeRoleOptions`) — exportar função de arquivo de componente quebra o fast-refresh, e o lint pega.
  - **Segunda rodada, motivada pelo teste de UI (ver "Defeitos que só a tela revelou"):** `mergeRoleOptions` passou a rotular perfil do tenant com **zero permissões** como `"— sem permissões"`, e a desambiguar rótulos que colidem mostrando o nome cru (`Vendedor (VENDEDOR)` / `Vendedor (vendedor)`).
  - `RolesPage.tsx` — o `div` que exibe `permissionsError` ganhou `role='alert'`. É ele que carrega a recusa do 409 ao desativar perfil em uso, e estava sem o atributo que `WriteError` (Financeiro) e `rowActionError` (Fornecedores) já usam.
  - `RolesPage.tsx` — o diálogo de edição ganhou Nome e Descrição, chamando `PATCH /roles/:id` — endpoint que **já existia** (`roles.controller.ts`) e não tinha chamador nenhum no frontend. Campos desabilitados para `isSystem`. O confirm de desativar passou a avisar que as permissões serão revogadas e que a exclusão é recusada se ainda houver usuários no perfil.
  - `Financeiro.tsx` — os dois `.catch(() => {})` do fetch de `/fornecedores` viraram o hook `useFornecedoresFiltro`: não chama a API sem `fornecedores.ver`, esconde o filtro nesse caso, e erro por outro motivo vira mensagem em vez de sumiço silencioso. O template `financeiro` **não** foi alterado de propósito — mexer nele altera `catalog.spec.ts` e o provisionamento de todo tenant.
- **Arquivos:** `backend/src/permissions/permissions.service.ts`, `backend/src/common/guards/permission.guard.ts`, `backend/src/roles/roles.service.ts`, `backend/src/roles/dto/create-role.dto.ts`, `backend/src/roles/dto/update-role.dto.ts`, `backend/src/users/users.service.ts`, `frontend/src/lib/authorization.ts`, `frontend/src/components/Can.tsx` (novo), `frontend/src/components/ProtectedRoute.tsx`, `frontend/src/App.tsx`, `frontend/src/components/layout/Sidebar.tsx`, `frontend/src/pages/{Dashboard,Pedidos,PedidoDetalhe,Clientes,Produtos,Fornecedores,Transporte,Financeiro}.tsx`, `frontend/src/pages/configuracoes/{UsuariosPage,RolesPage}.tsx`, `frontend/src/lib/roleOptions.ts` (novo)
- **Evidência:** `npm test --workspace=shared` → 13 passed (inalterado). `npm test --workspace=backend` → 59 suítes, **696 passed** (1 skipped; era 669), com casos novos para: 409 ao excluir perfil em uso **sem efeito colateral**; delete apagando `tenant_role_permissions`; auditoria com a contagem revogada; rename propagando e bumpando `access_token_version`; rename recusado em colisão **sem** propagar; guard negando role morta e `localUser.active === false`; `listEffectiveForRole` provando o join com `active`/`deleted_at`; corrida `23505` não devolvendo role soft-deleted. `npm test --workspace=frontend` → 24 arquivos, **157 passed** (era 109), com `Can.test.tsx` (6), `authorization.test.ts` (10, incluindo a regressão do bypass), `lib/rbacContract.test.ts` (18) — **teste de contrato** que varre `App.tsx` e `Sidebar.tsx` exigindo que toda rota/item de módulo declare `permission` e que todo slug citado exista em `PERMISSION_SLUGS` — e `lib/roleOptions.test.ts` (7), que fixa os dois defeitos achados na tela. Lint limpo nos dois workspaces; `npm run build --workspace=frontend` ok; `tsc --noEmit` limpo nos dois.
- **Evidência em runtime (Postgres de dev, porta 5433):** `db:migrate` + `db:verify` → "OK: schema íntegro. Nenhum drift encontrado." Script descartável levantando o AppModule e chamando `RolesService`/`PermissionsService` reais → **12/12 OK**, com limpeza confirmada por SQL depois (zero resíduo): permissões efetivas corretas com perfil vivo; 409 na exclusão de perfil em uso, sem alterar o perfil nem apagar os 2 vínculos; exclusão sem vinculados deixando `active=false` + `deleted_at` e **0 vínculos restantes**; permissões efetivas zeradas depois; **cenário legado** (perfil soft-deleted que ainda TEM vínculo, o estado de tudo que foi excluído antes desta correção) resolvendo para zero permissões; rename com `usuarios.roles` acompanhando (`["qa_renomeado"]`) e `access_token_version` indo de 1 para 2.
- **Evidência pela UI (Safari, sessão real de `admin@renowa.local`, 2026-08-26):** executado com `ops/qa-safari` (driver injetado na aba logada) mais um roteiro de perfis escrito para esta correção. `p12` — varredura de **15 telas, zero erro**, rodada duas vezes: antes e depois das correções da segunda rodada. Ciclo de perfil ponta a ponta, tudo por clique de tela: criar perfil sob medida em Perfis (2 permissões) → **ele aparece no select de novo usuário**, que é o que antes era impossível → editar Nome e Descrição pelo diálogo, com as permissões preservadas e o botão Editar do perfil de sistema `disabled` → criar usuário nesse perfil, que nasce com ele e não com um template → **desativar o perfil em uso é recusado na tela**, com o texto literal `Perfil em uso por 1 usuário(s). Mova-os para outro perfil antes de excluir.`, e o perfil segue intacto com as 2 permissões → mover o usuário para outro perfil pela tela de Usuários → desativar de novo, agora aceito, sumindo da lista e da tabela. Conferido por SQL depois: perfil com `active=false`, `deleted_at` preenchido e **0 vínculos**; usuário com `usuarios.roles = ["vendedor"]` e `access_token_version` em 2. Banco restaurado ao estado pré-teste (inclusive removendo a role `vendedor` que o próprio roteiro provisionou), resíduo zero conferido por consulta.
- **Defeitos que só a tela revelou — ambos introduzidos por esta própria correção, e corrigidos na mesma sessão:**
  1. **Perfil com zero permissões voltou a ser oferecido, calado.** Ao unir `GET /roles` com os templates, `UsuariosPage` passou a oferecer também os perfis legados do tenant — e o `viewer` do banco de dev tem **0 permissões**. Quem o escolhesse criaria um usuário que loga e toma 403 em toda tela: o sintoma exato que FIX-0028 fechou, reentrando por outra porta. Esconder não servia — voltaria a torná-lo inatribuível, que é o defeito que esta correção veio consertar. Agora aparece rotulado como `Viewer — sem permissões`.
  2. **Rótulos colidindo.** `formatRoleName` faz title-case, então `VENDEDOR` e `vendedor` — perfis distintos, com ids distintos no banco — renderizavam ambos como `Vendedor`, indistinguíveis no select. Colisão agora mostra o nome cru entre parênteses.
  Os dois estão fixados por `lib/roleOptions.test.ts` e revalidados na tela: o select passou a listar `Vendedor (VENDEDOR)`, `Vendedor (vendedor)` e `Viewer — sem permissões`, com `Administrador` intacto por não colidir.
- **Lição:** a suíte de mock, o `tsc`, o lint e até a verificação contra Postgres real passaram **verdes com os dois defeitos presentes** — nenhum deles olha para o que o select oferece. Foi preciso abrir a tela com dados reais de um tenant legado. Vale para a próxima correção de RBAC: `p12` prova que nada quebrou, mas quem roda como `admin` com as 32 permissões não exercita nenhum gate.
- **Efeito observável:** excluir perfil passou a ser revogação de verdade — e, se o perfil ainda tem gente, a exclusão é **recusada** com mensagem que diz quantos. Perfil sob medida virou atribuível na criação de usuário. Quem não tem a permissão não vê mais a rota, o item de menu nem o botão. Renomear perfil não deixa mais token carregando nome inexistente. O filtro de fornecedor no Financeiro deixou de sumir sem explicação para quem não tem `fornecedores.ver`.
...
### FIX-0036 — "Liberar pedido" descartava edição não salva (observação sumia)
- **Data:** 2026-09-21
- **Fecha:** nenhum PROB anterior. Abre BACKLOG-0093 (resíduo de navegação interna).
- **Área:** frontend
- **Sintoma:** na edição de pedido (interno e externo), o usuário digitava a observação, clicava em "Liberar pedido" e a tela travava com o texto visível, como se tivesse gravado. Ao reabrir o pedido, a observação tinha sumido. O mesmo valia para qualquer campo, item ou foto alterado e não salvo.
- **Causa raiz:** confirmada por teste. `handleLiberar` (`PedidoForm`, `PedidoExternoForm`) chamava só `PATCH /pedidos/:uuid/liberar`, sem gravar o formulário. O caminho "Salvar" (`PUT`) estava correto de ponta a ponta. Cliente, SAC, nota fiscal e inadimplência foram revisados e não têm o defeito.
- **Correção:** com edição pendente, o botão passa a se chamar "Salvar e liberar" e executa, em ordem: validação nativa do formulário (`reportValidity`, a mesma que o submit do "Salvar" recebe), `persist()` (o mesmo caminho do "Salvar": `PUT`, depois as operações de foto) e o `PATCH` de liberação com a `version` devolvida pelo save. O backend não muda. Um endpoint atômico "salvar + liberar" não resolveria o problema, porque as fotos são requisições separadas e o backend recusa foto em pedido liberado. Elas precisam subir antes. Fotos não alteram `pedidos.version`, então o encadeamento não gera 409 falso. Se o save falha, nada é liberado. Se o save passa e a liberação falha, o pedido fica `em_aberto` com a edição gravada, que é um estado válido. Os campos ficam congelados durante o save e a liberação, porque o que fosse digitado nessa janela não iria no payload. Edição pendente é detectada pelo novo hook `useUnsavedChanges`, que compara um snapshot com o último estado gravado e deixa de fora o estado derivado (`status`, preview e versão da foto). O hook também pede confirmação no `beforeunload`.
- **Arquivos:** `frontend/src/hooks/useUnsavedChanges.ts`, `frontend/src/hooks/useUnsavedChanges.spec.ts`, `frontend/src/pages/PedidoForm.tsx`, `frontend/src/pages/PedidoForm.spec.tsx`, `frontend/src/pages/PedidoExternoForm.tsx`, `frontend/src/pages/PedidoExternoForm.spec.tsx`
- **Evidência:** os testes novos de "Salvar e liberar" falharam antes da correção. O teste de validação nativa falha quando a guarda é removida. Depois da correção: `npm run lint --workspace=frontend` limpo, `npm run build --workspace=frontend` limpo, `npm test --workspace=frontend` → 44 arquivos, **287 passed**. Revisão independente (quality-reviewer): nenhum bloqueador; as ressalvas foram aplicadas. Na tela (Safari, `ops/qa-safari`, fase `p5b`, só interação de UI, sem API): 17/17 asserções. O pedido foi criado pelo menu "Novo Pedido → Pedido interno", com cliente e fornecedor escolhidos na lista. A observação salva pelo "Salvar" aparece no detalhe. O botão vira "Salvar e liberar" só com edição pendente. Depois de "Salvar e liberar", a tela trava, o detalhe mostra a observação nova e o status Liberado, e a edição reaberta mantém o texto. Por fim o pedido foi cancelado e excluído pelos botões do detalhe e sumiu da busca. Nenhum erro de console ou HTTP na rodada. O banco confirma `observacao` com o texto liberado e o pedido excluído.
- **Efeito observável:** o que foi digitado é gravado antes de liberar. Fechar a aba ou recarregar com edição pendente pede confirmação.
- **Ressalvas:** navegação **interna** (menu, links) com edição pendente ainda não avisa, porque `<BrowserRouter>` não suporta `useBlocker` (BACKLOG-0093). O aviso de `beforeunload` não foi exercitado no Safari. O diálogo nativo trava o AppleScript; ele está coberto no teste do hook.
- **commits:** `22197a8` (correção e testes unitários), `7017643` (QA Safari)
