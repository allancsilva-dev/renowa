...
### PROB-0084 — Excluir um perfil não revogava acesso: usuários vinculados seguiam passando por todo `PermissionGuard`
- **Data:** 2026-08-26
- **Origem:** auditoria de RBAC (entregue por outro agente) + verificação item a item contra o código
- **Severidade:** BLOCKER
- **Status:** FECHADO_COM_RESSALVA (2026-08-26) — corrigido em FIX-0031, verificado em runtime contra o Postgres de dev **e pela UI no Safari** (BACKLOG-0081, concluído). A UI foi exercitada nos dois ramos — como `admin` (BACKLOG-0081) e com perfil restrito de um único slug (BACKLOG-0085, concluído). Ressalva que resta: os vínculos órfãos anteriores à correção não foram saneados, e **nada foi commitado**.
- **Área:** segurança / backend
- **Sintoma:** o admin excluía um perfil pela tela de Perfis, o perfil sumia da lista, e os usuários vinculados continuavam com **todas** as permissões daquele perfil — indefinidamente. Nada invalidava o token; a UI também seguia mostrando as permissões fantasma, porque `GET /auth/me` é alimentado pelo mesmo caminho.
- **Causa raiz:** confirmada, e é a soma de três coisas. (1) `deleteRole` (`backend/src/roles/roles.service.ts`) só marcava `active = false` + `softDelete` — como é **soft** delete, o `ON DELETE CASCADE` de `tenant_role_permissions` (definido em `backend/src/database/migrations/0021_cross_tenant_foreign_keys.sql:142`) **nunca** dispara e os vínculos permanecem no banco. (2) `PermissionsService.listEffectiveForRole` consultava `tenant_role_permissions` **por `roleId`**, sem filtrar `active` nem `deleted_at` — então lia exatamente as linhas que ficaram. (3) `local_users.role_id` continuava apontando para a role morta, e nenhum `access_token_version` era incrementado.
- **Impacto técnico:** revogação de acesso não existia como operação. O admin acreditava ter revogado; o guard continuava aprovando. Agravante de rastreabilidade: o índice UNIQUE **parcial** `UQ_tenant_roles_tenant_id_name_active` (migration `0025`) permite recriar um perfil com o mesmo nome depois do soft-delete — dois grupos distintos, com o mesmo nome, invisivelmente separados.
- **Arquivos/módulos:** `backend/src/roles/roles.service.ts` (`deleteRole`), `backend/src/permissions/permissions.service.ts` (`listEffectiveForRole`), `backend/src/common/guards/permission.guard.ts`, `backend/src/database/migrations/0021_cross_tenant_foreign_keys.sql:142`, `backend/src/database/migrations/0025_*` (índice parcial)
- **Solução proposta:** ponto de estrangulamento único na leitura (a query efetiva exige role viva) **mais** limpeza real dos vínculos na exclusão, para não depender de a leitura estar sempre certa.
- **Solução aplicada:** FIX-0031. `listEffectiveForRole` virou QueryBuilder com `innerJoin` em `TenantRole` exigindo `role.active = true AND role.deleted_at IS NULL` — atende guard e `/auth/me` de uma vez, e **cobre inclusive os perfis já excluídos antes da correção**. `deleteRole` foi reescrito em transação: recusa com **409** citando a contagem se houver `local_users` ativos no perfil (decisão do usuário: fail-closed — mover usuário é ato deliberado do admin), e, sem vinculados, apaga as linhas de `tenant_role_permissions` do par `(tenantId, roleId)` antes do soft-delete. O guard passou a negar também quando `localUser.active === false`.
- **Evidências/comandos:** `npm test --workspace=backend` → 59 suítes, **696 passed** (1 skipped; era 669), com casos novos cobrindo cada aresta. `db:migrate` + `db:verify` no dev → "OK: schema íntegro. Nenhum drift encontrado." Script descartável levantando o AppModule e chamando `RolesService`/`PermissionsService` reais contra o Postgres de dev (porta 5433) → **12/12 OK**, com limpeza confirmada por SQL depois (zero resíduo): perfil criado com 2 permissões e permissões efetivas corretas com perfil vivo; exclusão recusada com 409 (`Perfil em uso por 1 usuário(s)...`) **sem** alterar o perfil nem apagar os 2 vínculos; sem vinculados, exclusão passa com `active=false` + `deleted_at` preenchido e **0 vínculos restantes**; permissões efetivas zeradas após a exclusão; e o **cenário legado** — perfil soft-deleted que ainda TEM vínculo no banco, que é o estado dos perfis excluídos antes desta correção — resolvendo para zero permissões pelo filtro de role viva.
- **Evidência pela UI (2026-08-26):** o mesmo ciclo, agora por clique de tela no Safari com sessão real de `admin@renowa.local` (`ops/qa-safari` + roteiro de perfis). Desativar perfil em uso é recusado **na tela**, com o texto `Perfil em uso por 1 usuário(s). Mova-os para outro perfil antes de excluir.`, e o perfil segue com as 2 permissões; movido o usuário, a desativação passa e o SQL confirma `active=false`, `deleted_at` preenchido e **0 vínculos**. `p12` — 15 telas, zero erro — rodada duas vezes. Detalhe e os dois defeitos que a tela revelou (ambos introduzidos por FIX-0031, corrigidos na mesma sessão) estão em FIX-0031 e em BACKLOG-0081.
- **Correções ao relatório de auditoria original (o relatório está errado nestes pontos):** `PATCH /roles/:id` **já existia** no backend (`backend/src/roles/roles.controller.ts:40`) — o que faltava era chamador no frontend, não endpoint. `ProtectedRoute` com `permission` **já existia** e cobria `/sac`, `/faturamento` e os formulários de pedido — o buraco era de cobertura, não de mecanismo. As migrations vão até **`0043`** (não `0042`), e **não existe** `0039`. As tabelas são `tenant_roles`/`tenant_role_permissions` — `role_permissions` foi dropada na `0022`, não existe `roles`/`role_permissions`. `npm` **existe** na máquina (11.17.0): o relatório alegou `npm: command not found` e por isso **não rodou teste nenhum** — nenhuma afirmação de comportamento dele estava verificada. O provisionamento sob demanda **tem** guarda (nome sem template → 400, FIX-0028). Confirmados pela verificação: os achados (1) exclusão sem revogação, (2) perfil custom não atribuível na criação de usuário, (3) duas fontes de verdade sem sincronia no rename, (4) frontend com RBAC parcial, (5) template `financeiro` sem `fornecedores.ver` com 403 engolido, (6) bypass de admin por nome de role, (7) DTO de role sem `@IsNotEmpty()`, (9) testes sem cobertura desses riscos. O catálogo de **32 slugs em 11 módulos** confere item a item, sem drift entre `shared/src/permissions/catalog.ts`, migrations e decorators.
- **Achado novo, fora do relatório:** `ensureTenantRoleWith` (`backend/src/users/users.service.ts`), no catch de `23505`, relia a role **sem** filtro `active` — a corrida podia devolver uma role soft-deleted e vincular um usuário novo a perfil morto. Corrigido no mesmo FIX.
