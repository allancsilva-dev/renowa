...
import { encontrarCodigosDuplicados, mensagemCodigosDuplicados, mensagemLinhaDuplicada } from '@/lib/orderItemCodes';
      ...
      <section className='rounded-lg border border-slate-200 bg-white p-5 shadow-sm'>
        ...
        <div className='space-y-4'>{items.map((item, index) => { const calculated = previewItem(item); return <div key={item.uuid} className={`rounded-lg border p-4 ${duplicados.uuids.has(item.uuid) ? 'border-red-300 bg-red-50/40' : item.precisa_produto ? 'border-amber-300 bg-amber-50/40' : 'border-slate-200'}`}>
          ...
          <div className='grid gap-3 md:grid-cols-4'>
            <label className='flex flex-col gap-1 md:col-span-2'><span className={labelClass}>Produto cadastrado</span><AsyncCombobox disabled={fieldsDisabled || !header.fornecedor_uuid} value={item.produto_uuid || null} displayValue={item.produto_uuid ? `${item.codigo_manual ? `${item.codigo_manual} — ` : ''}${item.descricao_manual}` : ''} ariaLabel={`Buscar produto do item ${index + 1}`} ariaInvalid={item.precisa_produto || duplicados.motivos.get(item.uuid) === 'produto'} placeholder={header.fornecedor_uuid ? 'Busque por código ou descrição' : 'Selecione o fornecedor primeiro'} emptyMessage='Nenhum produto desse fornecedor encontrado.' fetcher={productFetcher} onChange={(value, option) => chooseProduct(item.uuid, value, option)} className={`${inputClass}${item.precisa_produto ? ' border-amber-400' : ''}`} /></label>
            <div className='flex flex-col gap-1'>
              ...
              {duplicados.uuids.has(item.uuid) && <span id={`item-${item.uuid}-codigo-erro`} className='text-xs text-red-700'>{mensagemLinhaDuplicada(duplicados.motivos.get(item.uuid) ?? 'codigo')}</span>}
            </div>
